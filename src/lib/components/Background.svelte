<script lang="ts">
	...
	function generateRandomMatrix(m: Uint8Array[], bias: number, w: number, h: number) {
		for (let i = 0; i < h; i++)
			for (let j = 0; j < w; j++) if (!m[i][j]) m[i][j] = Math.round(Math.random() - bias);
		return m;
	}
	...
	function drawCanvas(
		...
		let data = erase ? new Uint8ClampedArray(w * h * 4).fill(0) : secondCanvasData;
	...
	function initCanvas(c: HTMLCanvasElement, ctx: CanvasRenderingContext2D): Animation {
		...
		const animation = c.animate(
			[{ opacity: firstTime ? 0 : hasFilter ? 0.5 : 0.2 }, { opacity: hasFilter ? 1 : 0.5 }],
