<script lang="ts">
	let { breadcrumbs, current }: { breadcrumbs: { title: string; url: string }[]; current: string } =
		$props();
...
<style>
	...
	.seg {
		position: relative;
		display: flex;
		flex: none; /* ancestors keep their (capped) size; only the last shrinks */
		max-width: 9rem;
		align-items: center;
	...
	/* Last: notch-left, straight right (the pill rounds it). It absorbs the
	   overflow — shrinks so its label ellipsizes instead of pushing past the edge. */
	.seg.last {
		flex: 0 1 auto;
		min-width: 0;
		padding-right: 1rem;
	...
	/* Current crumb fills the shrunk last segment and ellipsizes within it. */
	.seg.last .label {
		min-width: 0;
		max-width: 100%;
	}
