...
async function main() {
	...
	console.log(
		`[buildExcalidraw] Found ${excalidrawFiles.length} excalidraw file(s) in ${VAULT_DIR}`
	);
	...
	try {
		...
		console.error(
			'[buildExcalidraw] Could not start Chromium exporter (skipping all exports):',
			err.message
		);
		fs.writeFileSync(MAP_PATH, JSON.stringify(map, null, 2));
	...
	try {
		for (const absPath of excalidrawFiles) {
			...
			if (!lightSvg && !darkSvg) {
				console.error(
					`[buildExcalidraw] Both exports failed for ${baseName} (no live elements or error); skipping`
				);
				continue;
			}
